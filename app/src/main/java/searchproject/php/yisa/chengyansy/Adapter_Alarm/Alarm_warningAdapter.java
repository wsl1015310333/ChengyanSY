...
public class Alarm_warningAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Alarm_warningAdapter(List dataList , Context context) {
    ...
    private Alarm_warningAdapter.OnItemClickLitener mOnItemClickLitener;

    public void setOnItemClickLitener(Alarm_warningAdapter.OnItemClickLitener mOnItemClickLitener)
    {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            Alarm_warningAdapter.MyViewHolder myViewHolder = new Alarm_warningAdapter.MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_alarminfo, parent, false));
...
//
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof Alarm_warningAdapter.MyViewHolder){

            ((Alarm_warningAdapter.MyViewHolder)holder).plate.setText(dataList.get(position).getPlate()==null?"":dataList.get(position).getPlate().toString());
            ((MyViewHolder)holder).reson.setText(dataList.get(position).getReason()==null?"":dataList.get(position).getReason().toString());
            ((Alarm_warningAdapter.MyViewHolder)holder).locationName.setText(dataList.get(position).getLocationName()==null?"":dataList.get(position).getLocationName().toString());
            ((Alarm_warningAdapter.MyViewHolder)holder).captureTime.setText(dataList.get(position).getCaptureTime()==null?"":dataList.get(position).getCaptureTime().toString());
...
//                    .load("http://"+ip+":6253"+stringBuilder.substring(19))
                 //   .load("http://"+ HttpConnectionUtils.ipport+stringBuilder.substring(24))
                    ...
                    .resize(65, 50)
                    .centerCrop()
                    .into(((Alarm_warningAdapter.MyViewHolder)holder).imageView);
            if(mOnItemClickLitener!=null) {
                ((Alarm_warningAdapter.MyViewHolder) holder).imageView.setOnClickListener(new View.OnClickListener() {
                    @Override
        ...
        else if(holder instanceof Alarm_warningAdapter.FootViewHold){
            if(load_more_status==1){
                ((Alarm_warningAdapter.FootViewHold) holder).item_loadmore_container_loading.setVisibility(View.GONE);
                ((Alarm_warningAdapter.FootViewHold) holder).item_loadmore_container_retry.setVisibility(View.VISIBLE);
            }
            else if(load_more_status==0){
                ((Alarm_warningAdapter.FootViewHold) holder).item_loadmore_container_loading.setVisibility(View.VISIBLE);
                ((Alarm_warningAdapter.FootViewHold) holder).item_loadmore_container_retry.setVisibility(View.GONE);
            }
        }
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder{
       ...
       private ImageButton alarm_carinfo_acap;
        public MyViewHolder(View itemView) {
            ...
            alarm_carinfo_acap=(ImageButton)itemView.findViewById(R.id.alarm_carinfo_acap);
        }
    }
